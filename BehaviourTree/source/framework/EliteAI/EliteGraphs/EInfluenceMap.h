...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		for (const auto& node : m_Nodes)
		{
			...
			{
				float influence{ m_Nodes[connection->GetTo()]->GetInfluence() * expf(-connection->GetCost() * m_Decay)};

				if (abs(influence) > highestInfluence)
				{
					highestInfluence = abs(influence);
					float finalInfluence = Lerp(influence,node->GetInfluence(), m_Momentum);
					m_InfluenceDoubleBuffer[node->GetIndex()] = finalInfluence;
				}		
			}
		}
