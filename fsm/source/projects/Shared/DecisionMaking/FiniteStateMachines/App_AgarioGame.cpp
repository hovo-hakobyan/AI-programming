...
void App_AgarioGame::Start()
{
	...
	FleeBiggerAgentState* pFleeBiggerAgent = new FleeBiggerAgentState();
	m_pStates.push_back(pFleeBiggerAgent);
	...
	SeekFoodToWanderCondition* pSeekFoodToWander = new SeekFoodToWanderCondition();
	m_pConditions.push_back(pSeekFoodToWander);

	EvadeAgentCondition* pEvadeAgentCondition = new EvadeAgentCondition();
	m_pConditions.push_back(pEvadeAgentCondition);

	EvadeToWanderCondition* pEvadeToWander = new EvadeToWanderCondition();
	m_pConditions.push_back(pEvadeToWander);
	...
	pStateMachine->AddTransition(pWanderState, pFleeBiggerAgent, pEvadeAgentCondition);

	pStateMachine->AddTransition(pSeekFood, pWanderState, pSeekFoodToWander);
	pStateMachine->AddTransition(pSeekFood, pFleeBiggerAgent, pEvadeAgentCondition);
	

	pStateMachine->AddTransition(pFleeBiggerAgent, pWanderState, pEvadeToWander);
...
Blackboard* App_AgarioGame::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("EvadeAgent", static_cast<AgarioAgent*>(nullptr));
