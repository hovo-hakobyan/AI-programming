...
namespace Elite
{
	...
	inline Eulerianity EulerianPath<T_NodeType, T_ConnectionType>::IsEulerian() const
	{
		...
		for (auto n : nodes)
		{
			...
			if (connections.size() & 1) // elke odd getal eindigt op 1
			{
				++oddCount;
			}
		}
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		if (IsEulerian() == Eulerianity::eulerian)
		{
			currentIdx = nodes.front()->GetIndex();
		}
		else if (IsEulerian() == Eulerianity::semiEulerian)
		{
			for (auto n : nodes)
			{
				auto connections = m_pGraph->GetNodeConnections(n);
		...
		int tempIdx{};
		while (graphCopy->GetNodeConnections(currentIdx).size() > 0)
		{
			...
			graphCopy->RemoveConnection(tempIdx, currentIdx);
		}
		nodeStack.push(currentIdx);
